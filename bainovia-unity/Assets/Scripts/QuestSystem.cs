...
public class QuestSystem : MonoBehaviour
{
    ...
    void CheckQuestCompletion(Quest quest)
    {
        ...
        if (allComplete)
        {
            Debug.Log($"Quest {quest.questName} objectives complete!");
            CompleteQuest(quest.questId);
        }
    }

    /// <summary>
    /// Called by enemy Health when something with a targetId dies.
    /// </summary>
    public void OnEnemyKilled(string targetId)
    {
    ...
    /// <summary>
    /// Called by item pickups when an item is collected.
    /// </summary>
    ...
    /// <summary>
    /// Compact objective line for the HUD, e.g. "Kill the Frost Brute (0/1)".
    /// </summary>
    public string GetObjectiveText(string questId)
    {
        ...
        foreach (QuestObjective objective in quest.objectives)
        {
            sb.Append(objective.description);
            sb.Append(" (").Append(objective.currentCount).Append('/').Append(objective.targetCount).Append(')');
            sb.AppendLine();
        }
    ...
    public void LoadData(string jsonData)
    {
        ...
        QuestSaveData data = JsonConvert.DeserializeObject<QuestSaveData>(jsonData);
        if (data == null) return;
