...
public class SpiritCompanionSystem : MonoBehaviour
{
    ...
    {
        // Summon active spirits (7-9 keys; 1-6 are reserved for runes)
        for (int i = 0; i < collectedSpirits.Count && i < maxActiveSpirits; i++)
        {
            if (Input.GetKeyDown(KeyCode.Alpha7 + i))
            {
    ...
    {
        player?.ApplySpiritShield(Mathf.RoundToInt(power));
    }
    ...
    public void LoadData(string jsonData)
    {
        ...
        SpiritSaveData data = JsonConvert.DeserializeObject<SpiritSaveData>(jsonData);
        if (data == null) return;
